...
{
  if(pwe->YPrintLevel>1)cout<<"Call FitOnePhoton\n";
  ...
  if(pwe->YPrintLevel>0)
	...
	for(Int_t icc=0; icc<nRealClusts; icc++) {
		...
		if( clustCatag == 1 ) {
			...
			clust[icc].photon[0].pHitCluster=&clust[icc];
...
// 			photons[nPh].errEne  = clust[icc].photon[0].errEne  ;
		...
		//
			...
			for(Int_t kp=0; kp<2; kp++) 
			  {
			    clust[icc].photon[kp].pHitCluster=&clust[icc];
...
// 				photons[nPh].errEne  = clust[icc].photon[kp].errEne  ;
		...
		//
			...
			//
		  if(pwe->YPrintLevel>0)printf("clustCatag==0\n");
			Bool_t is2Photon;
			is2Photon = true;
			altClu = clust[icc] ;
			altClu.nPhoton   = 1 ;
			chiSq1 = FitOnePhoton(&altClu);
			altClu.photon[0].pHitCluster=&altClu;
			...
			if( chiSq1 < maxGood1PhChi2NDF * NDF1 ) {
				is2Photon = false ;
				if(pwe->YPrintLevel>0)printf("is2Photon = false chiSq1=%f < %f * %d \n",chiSq1,maxGood1PhChi2NDF,NDF1);
			}
			//
       ...
       //
			  if(pwe->YPrintLevel>0)
			    {printf(" catag=0 chiSq1=%f chiSq2=%f \n",chiSq1,chiSq2);};
			...
			if( !is2Photon ) 
			  {
			    ...
			    clust[icc].NDFchiSq=altClu.NDFchiSq=NDF1;
...
// 					photons[nPh].errYPos = clust[icc].photon[kp].errYPos ;
// 					photons[nPh].errEne  = clust[icc].photon[kp].errEne  ;
			      if(pwe->YPrintLevel>0)printf("photon[%d].energy=%f\n",nPh,photons[nPh].energy);
			      nPh ++ ;
			    }
...
TLorentzVector  Yiqun::mom(Int_t ph_num)
{
  ...
  HitCluster* clu=(HitCluster*) photons[ph_num].pHitCluster;
  if(clu)clu->EDepUpdate(pwe);
...
TLorentzVector  Yiqun::mom(PhotonHitFPD* phot)
{
  ...
  TVector3 uvec(0.,0.,0.);
  if(dist!=0)uvec=(1./dist)*vvec;
  HitCluster* clu=(HitCluster*) phot->pHitCluster;
  if(!clu)printf("cluster points to zero\n");
  if(clu)clu->EDepUpdate(pwe);
