...
class HitCluster : public TObject {
 ...
 public:
  ...
  Float_t        chiSquare;             // Chi-square of the fitting per NDF
  Int_t         NDFchiSq;             // Chi-square NDF from the fit
  ...
  TObjArray* ShadowPointers;
  void LinkShadowPointer(HitCluster* lclu);
  HitCluster() ;
  void AddCluster(HitCluster* p_bclu=0);
  ~HitCluster();
  Bool_t IsEUpdated;
  void EDepUpdate(WasExternal* pwe); 
  Bool_t IsEDepUpdated;
  void Clear(void) ;
  ...
  HitCluster& operator=(const HitCluster& rhs) 
      ...
      if (this != &rhs) 
	{
	  ...
	  IsEDepUpdated=rhs.IsEDepUpdated;
	};
      
      for(Int_t i=0; i<nPhoton; i++) 
	{
	  photon[i] = rhs.photon[i] ;
	  photon[i].pHitCluster=this;
	}
