...
HitCluster::~HitCluster()
{
  if(ShadowPointers)delete ShadowPointers;
  if(tow)delete tow;
};
...
void HitCluster::EDepUpdate(WasExternal* pwe)
{
  ...
  Float_t Eratio=1;
  if((!IsEDepUpdated)&&pwe->UseEDepCorrection)
    {
      if(pwe->UseEDepCorrection)
	  {
	    Eratio=pwe->EDepCorrection->Eval(pwe->CalibAtEnergy[NSTB-1])/pwe->EDepCorrection->Eval(energy);
	  };
	...
	for(int nph=0;nph<nPhoton;nph++)
	  {
	    photon[nph].energy=photon[nph].energy*Eratio;
	    photon[nph].errEne=photon[nph].errEne*Eratio;
	    //	    printf("photon %d energies updated by ratio %f to %f \n",nph,Eratio, photon[nph].energy); 
	  };
	//	printf("Set Is EDepUpdated true\n");
	IsEDepUpdated=true;
    };
...
{ 
  ...
  if(ShadowPointers)
    {
      delete ShadowPointers;
    };
  ShadowPointers=0;
  if(tow)delete tow;
  tow=new TObjArray(50);
}
void HitCluster::LinkShadowPointer(HitCluster* lclu)
{
  /*
    Each cluster may have an associated list of "shadow" clusters that
    are prepared with diffrent selected parameters. This was introduced
    to allow recording the dependence of sigmamax on energy cutoff for 
    log weighting.
    ShadowClusters live in Yiqun (and are managed in Yiqun).
    They are pointe to by these links.
    Links are not copied with the copy method.
    Clear deletes the link list;
   */
  if(!ShadowPointers)ShadowPointers=new TObjArray(20,0);
  if(ShadowPointers->GetEntries()<20)ShadowPointers->Add(lclu);
}
...
Bool_t HitCluster::UpdateEnergy()
{
  ...
  EnergyUpdated=true;
  FindClusterAxis();
  WasExternal* pwe=&(FitTower::we);
  EDepUpdate(pwe); 
  return true;
};
void HitCluster::Setcluster2Number(Int_t ClusterNumber )
{
  cluster2=ClusterNumber;
  if(tow==0)return;
  TIter next(tow);
  while(TowerFPD* hit=(TowerFPD*) next())
    {
      hit->cluster2=cluster2;
    };
